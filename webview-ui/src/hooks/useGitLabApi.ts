...
export const useGitLabApi = () => {
	...
	useEffect(() => {
		const handleMessage = (event: MessageEvent) => {
			...
			if (message.type === "response") {
				...
				setResponses((prev) => new Map(prev.set(requestType, { success, data, error, options })))
	...
	const getApiState = useCallback(
		<T>(requestType: string): ApiResponse<T> => {
			...
			return {
				...
				options: response?.options ? response?.options : null
			}
		},
		[responses, loading],
	)
	...
	const createCherryPickMR = useCallback(
		(projectId: number, options: CherryPickOptions) => {
	...
	const closeMergeRequest = useCallback(
		(projectId: number, mergeRequestIid: number, tempBranchName?: string) => {
